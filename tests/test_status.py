...
def test_status_remotely_started(vehicle: Vehicle):
    ...
    assert vehicle.outside_temp is not None
    assert isinstance(vehicle.outside_temp, Temperature)
    assert isinstance(vehicle.odometer, Distance)
    assert vehicle.outside_temp.c == 5.75
    assert str(vehicle.outside_temp) == "42.35°F"
    ...
    assert vehicle.fuel_level.percent == 0.74239368
    assert str(vehicle.odometer) == "65580.12 mi"


@mock_responses(status="status/off.json")
def test_status_off(vehicle):
    """Test status when vehicle is off."""
    # Temperatures
    ...
    # Fuel
    assert vehicle.fuel_level.percent == 0.72717624
